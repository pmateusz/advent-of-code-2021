        ...
        for signs in itertools.product([1, -1], repeat=3):
            for target in [first_target[0], first_target[1]]:
                ...
                if verify_points_in_place(s1, s2, matched):
                    print("Put in place %s to %s by %s %s shifting %s" % (
                    s2.label, s1.label, shuffling, signs, (x - a, y - b, z - c)))
                    return
                else:
...
def part2(scanners) -> int:
    ...
    for s1 in scanners:
        for s2 in scanners:
            distances.append(abs(s1.own_pos[0] - s2.own_pos[0]) + abs(s1.own_pos[1] - s2.own_pos[1]) + abs(
                s1.own_pos[2] - s2.own_pos[2]))
